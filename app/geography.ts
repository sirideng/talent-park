...
export function groundHeight(x:number,z:number){const hill=(cx:number,cz:number,sx:number,sz:number,h:number)=>h*Math.exp(-((x-cx)**2/sx**2+(z-cz)**2/sz**2));const relief=hill(28,-29,9,13,2.2)+hill(15,45,13,5,.7);const pathDistance=Math.min(distanceToPath(x,z,shoreWalk,true),distanceToPath(x,z,northWalk),distanceToPath(x,z,southWalk));return .6+relief*Math.min(1,Math.max(0,(pathDistance-1.5)/3))}
