...
export default function Home(){
 ...
 const events=useMemo<SceneEvents>(()=>({
  selectLocation:setSelectedLocation,selectPlace:setSelected,
  visit:(id)=>setVisited(v=>v.includes(id)?v:[...v,id]),
  nearby:setNearby,momentAvailable:setMomentAvailable,
  resting:(value,moment)=>{setResting(value);setActiveMoment(value?moment:null);if(value)setNight(true)},
  resetView:()=>{setWalking(false);setNight(false);setSelected(null);setNearby(null);setMomentAvailable(null);setResting(false);setActiveMoment(null)},
 }),[]);
 const {sceneId,ready,error,definition,navigate}=useScene(mount,events,api);

 const mode=useCallback((value:boolean)=>{setWalking(value);api.current?.setWalking?.(value);setSelected(null)},[api]);
 ...
 function returnToPlanet(){navigate('planet')}

 useEffect(()=>{if(!ready||sceneId!=='talent-park')return;const context=(document as Document&{modelContext?:ModelContext}).modelContext;if(!context?.registerTool)return;const lifecycle=new AbortController();const definitions=[{name:'read_park_state',description:'读取当前公园视角、日夜状态、人物位置和已抵达的景点。',inputSchema:{type:'object',properties:{},additionalProperties:false},annotations:{readOnlyHint:true},execute:()=>api.current?.getState?.()},{name:'configure_park_view',description:'切换俯瞰或步行视角与日夜效果，与页面按钮同步。不会替玩家打卡。',inputSchema:{type:'object',properties:{mode:{type:'string',enum:['overview','walk']},night:{type:'boolean'}},required:['mode','night'],additionalProperties:false},annotations:{readOnlyHint:false},execute:async(input:unknown)=>{if(!isViewInput(input))throw new Error('请输入有效的 mode 和 night');mode(input.mode==='walk');setNight(input.night);api.current?.setNight?.(input.night);await new Promise<void>(resolve=>requestAnimationFrame(()=>resolve()));return api.current?.getState?.()}}];definitions.forEach(tool=>{try{Promise.resolve(context.registerTool(tool,{signal:lifecycle.signal})).catch(()=>{})}catch{}});return()=>lifecycle.abort()},[ready,sceneId,api,mode]);

 const planet=definition.layout==='globe';
 return <main className={`park-app memory-world${planet?' planet-mode':''} ${definition.ui?.className??''}${night?' night':''}${walking?' walking':''}${resting?' sitting':''}`}>
  <div ref={mount} className="scene" aria-label={definition.ariaLabel} data-scene-id={sceneId} data-scene-status={ready?'ready':error?'error':'loading'}/>
  <header className="masthead"><div className="brand-mark">{planet?<Globe2 size={26}/>:<Compass size={26}/>}</div><div><div className="eyebrow">{planet?'MY SHENZHEN · MEMORY PLANET':definition.ui?.eyebrow??'SHENZHEN · POCKET PARK'}</div><h1>{definition.name}<span>{planet?'一颗慢慢长大的星球':'微缩漫游'}</span></h1></div></header>
  {!planet&&<button className="back-to-planet" onClick={returnToPlanet}><Globe2 size={16}/>返回记忆星球</button>}
  {planet?<>
   <section className="planet-intro"><span className="chapter">SHENZHEN · 22.54° N</span><h2>把走过的地方，<br/>放进一颗星球。</h2><p>五座记忆岛，藏在同一片海里。轻轻转动，寻找熟悉的轮廓。</p></section>
   <div className="planet-status"><span className="live-dot"/><span>{PLANET_LOCATIONS.filter(location=>location.status==='ready').length} 个地点已经展开</span></div>
   <section className="location-card" aria-live="polite"><div className="eyebrow">MEMORY {selectedLocation.chapter} · {selectedLocation.status==='ready'?'可进入':'正在生长'}</div><h3>{selectedLocation.name}</h3><p>{selectedLocation.description}</p>{selectedLocation.status==='ready'?<button onClick={()=>api.current?.enterLocation?.(selectedLocation)}><Footprints size={17}/>走进这段记忆</button>:<span className="planned-note">这段记忆正在生长，漫游尚未开放</span>}</section>
   ...
   <nav className="memory-route" aria-label="深圳记忆地点">{PLANET_LOCATIONS.map(location=><button key={location.id} className={`${location.status} ${selectedLocation.id===location.id?'selected':''}`} onFocus={()=>api.current?.focusLocation?.(location)} onBlur={()=>api.current?.previewLocation?.(null)} onClick={()=>{setSelectedLocation(location);if(location.status==='ready')api.current?.enterLocation?.(location);else api.current?.focusLocation?.(location)}}><span>{location.chapter}</span>{location.shortName}</button>)}</nav>
  ...
  {!ready&&<div className="loading"><Compass size={36}/><p>{error||definition.loadingMessage}</p>{error&&<button onClick={()=>navigate(sceneId)}>重新加载</button>}</div>}
 </main>;
}
