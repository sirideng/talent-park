...
export default function Home(){
 ...
 return <main className={`park-app memory-world${planet?' planet-mode':''}${night?' night':''}${walking?' walking':''}${resting?' sitting':''}`}>
  ...
  {planet?<>
   ...
   <footer className="bottom-bar"><div className="mode-controls"><button className={!walking?'active':''} onClick={()=>mode(false)}><Compass size={18}/>俯瞰公园</button><button className={walking?'active':''} onClick={()=>mode(true)}><Footprints size={18}/>进入漫游</button></div><div className="instruction">{walking?'W A S D / 方向键移动 · Shift 加速 · 空格跳跃 · 可走到文化广场与周边街区':'拖动旋转 · 滚轮缩放 · 点击地标探索'}</div><div className="stamp-count"><MapPin size={17}/><strong>{visited.length}<span> / 6</span></strong><span>漫游印记</span></div></footer>
