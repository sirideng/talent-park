...
void main(){float ripple=wave(vWorld.x*3.+vWorld.z*1.6+time*.6)*wave(vWorld.z*3.4-vWorld.x*.9-time*.4);float phase=vWorld.x*2.8+vWorld.z*4.1+time*.6;float glint=pow(max(0.,sin(phase)),12.)*.025*(1.-smoothstep(.15,1.,fwidth(phase)));vec3 c=vec3(.19,.39,.38)+ripple*.012+glint;float band=exp(-pow((vWorld.x+14.+wave(vWorld.z*8.+time)*.22)/2.8,2.))*exp(-abs(vWorld.z+8.)*.025);c=mix(c,vec3(.055,.13,.18)+band*vec3(.23,.21,.11)*(0.45+0.15*wave(vWorld.z*9.+time)),night);gl_FragColor=vec4(c,1.); }`});
 ...
 const raycaster=new THREE.Raycaster(),pointer=new THREE.Vector2();let downX=0,downY=0,lastPointerY=0,viewDragging=false,lookOffset=0,lookTarget=0,walkDistance=10.8;let gestureAxis:'pending'|'horizontal'|'vertical'='pending';
 const pointerDown=(e:PointerEvent)=>{downX=e.clientX;downY=e.clientY;lastPointerY=e.clientY;gestureAxis='pending';viewDragging=walking&&!sitting&&!transitioning};
 ...
 const pointerMove=(e:PointerEvent)=>{if(!viewDragging||!walking||sitting||transitioning)return;const totalX=e.clientX-downX,totalY=e.clientY-downY;if(gestureAxis==='pending'&&Math.hypot(totalX,totalY)>5)gestureAxis=Math.abs(totalY)>Math.abs(totalX)*1.12?'vertical':'horizontal';const dy=lastPointerY-e.clientY;lastPointerY=e.clientY;if(gestureAxis!=='vertical')return;e.preventDefault();e.stopImmediatePropagation();lookTarget=THREE.MathUtils.clamp(lookTarget+dy*.045,-.5,15)};
 const pointerUp=(e:PointerEvent)=>{viewDragging=false;gestureAxis='pending';if(Math.hypot(e.clientX-downX,e.clientY-downY)>6)return;const r=renderer.domElement.getBoundingClientRect();pointer.set((e.clientX-r.left)/r.width*2-1,-(e.clientY-r.top)/r.height*2+1);raycaster.setFromCamera(pointer,camera);const hits=raycaster.intersectObjects(pickables.filter(o=>o.visible&&o.parent?.visible));if(hits.length){if(walking)interact();else onSelect(hits[0].object.userData.place)}};
 const pointerCancel=()=>{viewDragging=false;gestureAxis='pending'};
 renderer.domElement.addEventListener('pointerdown',pointerDown);renderer.domElement.addEventListener('pointermove',pointerMove,true);renderer.domElement.addEventListener('pointerup',pointerUp);renderer.domElement.addEventListener('pointercancel',pointerCancel);
 ...
 function reset(immediate=false){
  keys.clear();lookOffset=0;lookTarget=0;walkDistance=10.8;controls.enableDamping=false;controls.update();controls.enableDamping=true;
 ...
 markerGroups.forEach(g=>{g.position.y=walking?3.2:4.5;g.visible=!walking;});contextLabels.forEach(label=>label.visible=!walking);if(!transitioning){controls.update();if(walking&&!sitting){const next=reducedMotion.matches?lookTarget:THREE.MathUtils.damp(lookOffset,lookTarget,7.5,dt),lookDelta=next-lookOffset;controls.target.y+=lookDelta;lookOffset=next;const horizontal=new THREE.Vector2(camera.position.x-controls.target.x,camera.position.z-controls.target.z),current=Math.max(.01,horizontal.length());if(!viewDragging&&Math.abs(lookTarget-lookOffset)<.02)walkDistance=THREE.MathUtils.clamp(current-Math.max(0,lookOffset)*2.15,8,25);const desired=walkDistance+Math.max(0,lookOffset)*2.15,distance=reducedMotion.matches?desired:THREE.MathUtils.damp(current,desired,7,dt);horizontal.multiplyScalar(distance/current);camera.position.x=controls.target.x+horizontal.x;camera.position.z=controls.target.z+horizontal.y;const cameraFloor=floorHeight(camera.position.x,camera.position.z)+.42;if(camera.position.y<cameraFloor)camera.position.y=cameraFloor;camera.lookAt(controls.target)}}else camera.lookAt(controls.target);renderer.render(scene,camera)}frame=requestAnimationFrame(animate);
 return {setWalking,setNight,setSitting,setSound,interact,reset:()=>reset(),zoom:(f:number)=>{if(transitioning)return;const v=camera.position.clone().sub(controls.target);v.multiplyScalar(f).clampLength(controls.minDistance,controls.maxDistance);camera.position.copy(controls.target).add(v);controls.update()},key:(k:string,pressed:boolean)=>pressed?keys.add(k.toLowerCase()):keys.delete(k.toLowerCase()),getState:()=>({walking,sitting,soundOn,night,nightBlend,transitioning,visited:[...visited],moment:activeMoment?.id??null,position:{x:player.position.x,z:player.position.z}}),dispose:()=>{dead=true;cancelAnimationFrame(frame);observer.disconnect();controls.dispose();audio?.sources.forEach(source=>source.stop());void audio?.context.close();window.removeEventListener('keydown',keyDown);window.removeEventListener('keyup',keyUp);window.removeEventListener('blur',blur);renderer.domElement.removeEventListener('pointerdown',pointerDown);renderer.domElement.removeEventListener('pointermove',pointerMove,true);renderer.domElement.removeEventListener('pointerup',pointerUp);renderer.domElement.removeEventListener('pointercancel',pointerCancel);const geometries=new Set<THREE.BufferGeometry>(),materials=new Set<THREE.Material>();scene.traverse(o=>{if(o instanceof THREE.Sprite){materials.add(o.material)}if(o instanceof THREE.Mesh||o instanceof THREE.Points){geometries.add(o.geometry);(Array.isArray(o.material)?o.material:[o.material]).forEach(m=>materials.add(m))}});geometries.forEach(g=>g.dispose());materials.forEach(m=>m.dispose());textures.forEach(t=>t.dispose());renderer.dispose();renderer.domElement.remove()}}
}
