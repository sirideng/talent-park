...
export function createPark(host:HTMLElement,onSelect:(p:typeof PLACES[number])=>void,onVisit:(id:string)=>void,onNearby:(p:typeof PLACES[number]|null)=>void){
 ...
 const camera=new THREE.PerspectiveCamera(42,host.clientWidth/host.clientHeight,.5,400);
...
void main(){float ripple=wave(vWorld.x*3.+vWorld.z*1.6+time*.6)*wave(vWorld.z*3.4-vWorld.x*.9-time*.4);float phase=vWorld.x*2.8+vWorld.z*4.1+time*.6;float glint=pow(max(0.,sin(phase)),12.)*.025*(1.-smoothstep(.15,1.,fwidth(phase)));vec3 c=vec3(.19,.39,.38)+ripple*.012+glint;float band=exp(-pow((vWorld.x+14.+wave(vWorld.z*8.+time)*.22)/2.8,2.))*exp(-abs(vWorld.z+8.)*.025);c=mix(c,vec3(.055,.13,.18)+band*vec3(.23,.21,.11)*(0.45+0.15*wave(vWorld.z*9.+time)),night);gl_FragColor=vec4(c,1.); }`});
 ...
 function tree(x:number,z:number,scale=1){const y=groundHeight(x,z);trunks.push(new THREE.Matrix4().compose(new THREE.Vector3(x,y+scale,z),new THREE.Quaternion(),new THREE.Vector3(.10*scale,2*scale,.1*scale)));for(let j=0;j<3;j++){temp.position.set(x+(j-1)*.36*scale,y+(2.1+(j%2)*.45)*scale,z+(j%2)*.35*scale);temp.rotation.set(0,rand()*6,0);temp.scale.set(.9*scale,1.0*scale,.9*scale);temp.updateMatrix();crowns.push(temp.matrix.clone());colors.push(new THREE.Color().setHSL(.26+rand()*.05,.25+rand()*.13,.28+rand()*.12))}obstacles.push({x,z,r:.25*scale})}
 ...
 const pickables:THREE.Object3D[]=[],markerGroups:THREE.Group[]=[];const textures:THREE.Texture[]=[digitsTexture];
 ...
 let walking=false,night=false,frame=0,last=performance.now(),elapsed=0,nearbyId='',dead=false;
 ...
 function reset(immediate=false){
  ...
  controls.minDistance=.5;controls.maxDistance=300;controls.enabled=false;transitioning=true;
  if(immediate||reducedMotion.matches){camera.position.copy(cameraGoal);controls.target.copy(targetGoal);finishTransition()}
 }
 function finishTransition(){transitioning=false;controls.minDistance=walking?5:40;controls.maxDistance=walking?25:210;controls.enabled=true;controls.update()}
 function setNight(value:boolean){night=value}
 function updateAtmosphere(dt:number){
  nightBlend=reducedMotion.matches?Number(night):THREE.MathUtils.damp(nightBlend,Number(night),.85,dt);
 ...
 if(walking&&!transitioning){let dx=Number(keys.has('d')||keys.has('arrowright'))-Number(keys.has('a')||keys.has('arrowleft')),dz=Number(keys.has('s')||keys.has('arrowdown'))-Number(keys.has('w')||keys.has('arrowup'));if(dx||dz){const forward=camera.position.clone().sub(controls.target);forward.y=0;forward.normalize();const right=new THREE.Vector3(forward.z,0,-forward.x);const move=forward.multiplyScalar(dz).add(right.multiplyScalar(dx)).normalize().multiplyScalar(dt*(keys.has('shift')?7:3.8));const before=player.position.clone();if(canWalk(player.position.x+move.x,player.position.z))player.position.x+=move.x;if(canWalk(player.position.x,player.position.z+move.z))player.position.z+=move.z;player.rotation.y=Math.atan2(move.x,move.z)+Math.PI;const delta=player.position.clone().sub(before);camera.position.add(delta);controls.target.add(delta);legs[0].rotation.x=Math.sin(elapsed*12)*.5;legs[1].rotation.x=-legs[0].rotation.x;body.position.y=.85+Math.abs(Math.sin(elapsed*12))*.035}else{legs.forEach(l=>l.rotation.x=0)}
 const oldY=player.position.y;const floorY=(bridgeHeight(player.position.x,player.position.z)??groundHeight(player.position.x,player.position.z))+.05;if(airborne){player.position.y+=jumpVelocity*dt-9*dt*dt;jumpVelocity-=18*dt;if(player.position.y<=floorY){player.position.y=floorY;jumpVelocity=0;airborne=false}}else player.position.y=floorY;halo.position.y=floorY-player.position.y+.02;if(airborne){legs[0].rotation.x=-.35;legs[1].rotation.x=.25;body.position.y=.85}camera.position.y+=player.position.y-oldY;controls.target.y+=player.position.y-oldY;
 const nearby=nearbyPlace();if((nearby?.id??'')!==nearbyId){nearbyId=nearby?.id??'';onNearby(nearby)}}
 markerGroups.forEach((g,i)=>{g.position.y=walking?3.2:4.5;g.visible=!walking;});if(!transitioning)controls.update();else camera.lookAt(controls.target);renderer.render(scene,camera)}frame=requestAnimationFrame(animate);
 return {setWalking,setNight,interact,reset:()=>reset(),zoom:(f:number)=>{if(transitioning)return;const v=camera.position.clone().sub(controls.target);v.multiplyScalar(f).clampLength(controls.minDistance,controls.maxDistance);camera.position.copy(controls.target).add(v);controls.update()},key:(k:string,pressed:boolean)=>pressed?keys.add(k.toLowerCase()):keys.delete(k.toLowerCase()),getState:()=>({walking,night,nightBlend,transitioning,visited:[...visited],position:{x:player.position.x,z:player.position.z}}),dispose:()=>{dead=true;cancelAnimationFrame(frame);observer.disconnect();controls.dispose();window.removeEventListener('keydown',keyDown);window.removeEventListener('keyup',keyUp);window.removeEventListener('blur',blur);renderer.domElement.removeEventListener('pointerdown',pointerDown);renderer.domElement.removeEventListener('pointerup',pointerUp);const geometries=new Set<THREE.BufferGeometry>(),materials=new Set<THREE.Material>();scene.traverse(o=>{if(o instanceof THREE.Sprite){materials.add(o.material)}if(o instanceof THREE.Mesh){geometries.add(o.geometry);(Array.isArray(o.material)?o.material:[o.material]).forEach(m=>materials.add(m))}});geometries.forEach(g=>g.dispose());materials.forEach(m=>m.dispose());textures.forEach(t=>t.dispose());renderer.dispose();renderer.domElement.remove()}}
}
